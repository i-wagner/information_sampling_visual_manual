...
elseif iscell(x)
    tf = false(size(x));
    for k = 1:numel(x)
        tf(k) = (simplePolygonArea_mr(x{k}, y{k}) >= 0);
    end
else
    checkxy_mr(x, y, mfilename, 'X', 'Y', 1, 2)
    [first, last] = findFirstLastNonNan_mr(x);
    ...
    for k = 1:numParts
        s = first(k);
        e = last(k);
        tf(k) = (simplePolygonArea_mr(x(s:e), y(s:e)) >= 0);
    end
end
