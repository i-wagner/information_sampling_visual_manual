function epar = exp_trial_response(epar, el, tn)
    ...
    %% DEBUG; show AOIs
%     Screen('DrawLine', epar.windows, epar.black, ...
%            0,                 (min(epar.y) + ((max(epar.y) - min(epar.y)) / 2)) / epar.YPIX2DEG, ...
%            epar.SCREEN_X_PIX, (min(epar.y) + ((max(epar.y) - min(epar.y)) / 2)) / epar.YPIX2DEG)
%     Screen('DrawLine', epar.windows, epar.black, ...
%            epar.x_center, 0, ...
