...
%% Fitting options
model.minSearch.options = optimset('MaxFunEvals', 1, ...
                                   'MaxIter',     1, ...
                                   'TolFun',      1e-12, ...
                                   'TolX',        1e-12);
model.minSearch.init    = [0.50 0.10];
model.minSearch.min     = [0    0];
model.minSearch.max     = [1    2];
...
for s = 3:noSub % Subject
    ...
    [pred_propChoices(:, :, s), pred_noFix(:, :, :, s)] = ...
        test_unsystematic_fixations_3(setSizes, squeeze(emp_gain(s, :, :)), freeParameter(s, :));
...
pred_propFixSet2 = pred_noFix(:, 1:2, 2, :) ./ pred_noFix(:, 3, 2, :); % On chosen/not-chosen
% pred_propFixSet2 = pred_noFix(:, 1:2, 1, :) ./ pred_noFix(:, 3, 1, :); % On easy/difficult
...
for s = 3:noSub % Subject
    ...
    plot(0:8, emp_propFixChosenSet_ss(s, :)',  '-r', ...    % Empirical
         0:8, pred_propFixSet2(:, 1, 1, s), '-b', ... % Predicted
         'LineWidth', 2);
    hold off
    axis([-1 9 0 1])
    xticks(0:1:8)
...
for ss = 1:size(emp_propFixChosenSet_ss, 2) % Set size
    ...
    nexttile(ss);
    scatter(emp_propFixChosenSet_ss(:, ss), squeeze(pred_propFixSet2(ss, 1, 1, :)), ...
            'MarkerFaceColor', [0 0 0], ...
            'MarkerEdgeColor', [1 1 1])
    line([0 1], [0 1])
    axis square
