function [out_propChoices, out_fixNum] = test_unsystematic_fixations_3(sze_all, gain, freeParameter)
...
%     sze = [2 8];%[2 8];
    ...
    %%
    out_fixNum_chosen   = NaN(no_noiseSamples, 3, noSs);
    out_fixNum_easyDiff = NaN(no_noiseSamples, 3, noSs);
    out_propChoices     = NaN(no_noiseSamples, 2, noSs);
    for ns = 1:no_noiseSamples

        % Translate relative gain into choice bias
        gain_relative = gain(:, 2) - gain(:, 1);
        noise         = randn(1, 1) .* freeParameter(2);
        gain_relative = gain_relative + noise;

        pref_all = cdf('Normal', gain_relative, 0, freeParameter(1)) .* 2;
    ...
    %         fprintf('Fixations (Chosen, Nonchosen, both, check):\t%.2f,\t%.2f,\t%.2f,\t%.2f\n',chosen,nonChosen,fixNum3,chosen+nonChosen);
            ...
            % Temp output
            out_fixNum_easyDiff(ns, :, ss) = [fixNum1 fixNum2 sum([fixNum1 fixNum2])];
            out_fixNum_chosen(ns, :, ss)   = [chosen nonChosen sum([chosen nonChosen])];
            out_propChoices(ns, :, ss)     = [tg1 tg2];
    ...
    %% Output
    out_fixNum      = cat(3, squeeze(mean(out_fixNum_easyDiff))', ...
                             squeeze(mean(out_fixNum_chosen))');
    out_propChoices = squeeze(mean(out_propChoices))';
