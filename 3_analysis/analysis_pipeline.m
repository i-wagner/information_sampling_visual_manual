...
data.ss.fixations.propTrialOneAoiFix = ...
    getProportions(exper, anal, data.ss.fixations.atLeastOneFixatedAoi, ....
                   quality.nValidTrials, []);

%% Get time-related variables
% Trial durations
data.ss.time.trialDurations = ...
    getTrialDurations(exper, anal, data.ss.log.nCompletedTrials, data.ss.gaze);
...
data.ss.time.lostTime = ...
    getLostTime(exper, anal, quality.excludedTrials, data.ss.time.trialDurations);
...
compareVariableOfInterest(data.ss.fixations.propTrialOneAoiFix, ...
