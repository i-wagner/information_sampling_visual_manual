...
exper.num.conds  = (2)'; % 2 == single-target, 3 == double-target
exper.num.subs   = (1:99)';
...
%                 infSampling_getDwellTime(inp_gazeShifts, inp_stimOn, inp_stimOff, stim.identifier, stim.identifier_bg, curr_cond-2);
        ...
        clear curr_sub trial gazeShifts_allTrials choice_target time_planning time_inspection time_decision
...
if plt.save == 1
    ...
    inp_dat     = cellfun(@(x) x(:, [13 15 end]), sacc.gazeShifts(li_notExcld, :), 'UniformOutput', false);
...
sacc.propGs.onAOI_ss = NaN(exper.num.subNo, 9, 5, exper.num.condNo);
for c = 1:exper.num.condNo % Condition
    ...
    for s = 1:exper.num.subNo % Subject
        ...
        if ~isempty(gs_sub)
            ...
            for ss = 1:numel(setSizes) % Set size
                ...
                li_gsOnChosenTarget    = fixatedStim_ss == stim.identifier(1, id_chosenTarget_ss)';
                li_gsOnNonChosenTarget = fixatedStim_ss == stim.identifier(1, id_nonChosenTarget_ss)';
                li_gsOnChosenDist      = fixatedStim_ss == stim.identifier(2, id_chosenTarget_ss)';
                li_gsOnNonChosenDist   = fixatedStim_ss == stim.identifier(2, id_nonChosenTarget_ss)';
...
clc; matlab_pairedTtest(perf.score.final(:, 1), perf.score.final(:, 2))
...
% clc; round(squeeze(mean(sacc.time.search_ss(2, :, :), 2))', 2)                                                                  % Search time of extreme and remaining subjects
clc; disp([round(squeeze(mean(mean(sacc.time.search_ss, 2), 'omitnan'))', 2); ...  
           round(squeeze(mean(mean(sacc.time.search_ss, 2), 'omitnan'))'-ci_mean(squeeze(mean(sacc.time.search_ss, 2))), 2); ... 
           round(squeeze(mean(mean(sacc.time.search_ss, 2), 'omitnan'))'+ci_mean(squeeze(mean(sacc.time.search_ss, 2))), 2)])
...
clc; disp([round(mean(temp, 1, 'omitnan'), 2); ...                                                                                 3A: Proportion gaze shifts on AOIs
           ...
           round(mean(temp, 1, 'omitnan')+ci_mean(temp), 2)])
clear temp
clc; disp([round(mean(sacc.time.search_reg_coeff(:, :, 1), 'omitnan'), 2); ...                                                     3B: Search time over set sizes
           round(mean(sacc.time.search_reg_coeff(:, :, 1), 'omitnan')-ci_mean(sacc.time.search_reg_coeff(:, :, 1)), 2); ... 
           round(mean(sacc.time.search_reg_coeff(:, :, 1), 'omitnan')+ci_mean(sacc.time.search_reg_coeff(:, :, 1)), 2)])
clc; matlab_oneSampleTtest(sacc.time.search_reg_coeff(:, 2, 1));
clc; disp([round(mean(sacc.time.mean.inspection(:, 1, 1), 'omitnan'), 2); ...
           round(mean(sacc.time.mean.inspection(:, 1, 1), 'omitnan')-ci_mean(sacc.time.mean.inspection(:, 1, 1)), 2); ... 
           round(mean(sacc.time.mean.inspection(:, 1, 1), 'omitnan')+ci_mean(sacc.time.mean.inspection(:, 1, 1)), 2)])
clc; matlab_pairedTtest(perf.hitrates(:, 1, 2),             perf.hitrates(:, 1, 3))                                              % 3C: Discrimination difficulty
clc; matlab_pairedTtest(sacc.time.mean.planning(:, 1, 2),   sacc.time.mean.planning(:, 1, 3));                                   % 3D: Planning time
clc; matlab_pairedTtest(sacc.time.mean.inspection(:, 1, 2), sacc.time.mean.inspection(:, 1, 3));                                 % 3E: Inspection time
clc; matlab_pairedTtest(sacc.time.mean.decision(:, 1, 2),   sacc.time.mean.decision(:, 1, 3));                                   % 3F: Decision time

% Figure 4
% Intercepts and slopes of regression fit
clc; matlab_oneSampleTtest(model_io.reg.fit(:, 1)); % Intercepts
clc; matlab_oneSampleTtest(model_io.reg.fit(:, 2)); % Slopes
...
clc; matlab_pairedTtest(single_subjects(:, 1, 1), single_subjects(:, 2, 1)) % 5A: Proportions to chosen/not-chosen sets for first two gaze shifts
clc; matlab_pairedTtest(single_subjects(:, 1, 2), single_subjects(:, 2, 2)) % 5B: Proportions to easy/difficult sets for first two gaze shifts
clc; matlab_pairedTtest(single_subjects(:, 1, 3), single_subjects(:, 2, 3)) % 5C: Proportion to smaller/larger sets for first two gaze shifts
clc; matlab_pairedTtest(single_subjects(:, 1, 4), single_subjects(:, 2, 4)) % 5D: Proportions to closet/more distant sets for first two gaze shifts
clear single_subjects

% Figure 6
% Model results
clc; matlab_pairedTtest(model_io.data.double.perf, model_io.model.perf_perfect(:, 3))              % 6A: Empirical vs. maximum gain
...
clc; matlab_pairedTtest(model.freeParameter{2}(:, 1), model.freeParameter{2}(:, 2))                % 6B: Free parameters distributions
clc; matlab_pairedTtest(model_io.data.double.perf, model.performance(:, 2))                        % 6C: Empirical vs. stochastic model gain
...
inp_propGsOnAoi = [(1:5)' squeeze(mean(sacc.propGs.onAOI_ss(:, :, :, 1), 2, 'omitnan'))'];
inp_perf        = cat(3, ...
...
inp_dat_gs   = [(1:5)' squeeze(mean(sacc.propGs.onAOI_ss(:, :, :, 2), 2, 'omitnan'))'];
