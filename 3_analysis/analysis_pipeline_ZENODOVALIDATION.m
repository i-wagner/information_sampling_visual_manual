...
log.col.trialNo      = 30;     % Trial #
log.col.targetDiff   = 20;     % Target type shown in trial (only single-target experiment); 1 == easy, 2 == difficult
log.col.gapPosEasy   = 23;     % Gap location on easy target; 1 == bottom, 2 == top, 3 == left, 4 == right
log.col.gapPosHard   = 24;    % Gap location on difficult target
log.col.gapPosReport = 25;    % Reported gap location
log.col.noDisEasy    = 21;    % # of easy distractors in trial
log.col.noDisHard    = 22;    % # of difficult distractors in trial
log.col.stimPosX     = 31:48; % Positions on x-axis
log.col.stimPosY     = 49:66; % Positions on y_axis
log.col.hitMiss      = 26;    % Hit/miss
log.col.score        = 27;    % Number of points
log.col.fixErr       = 5;    % Flag for fixation error
...
for c = 1:exper.num.condNo % Condition
    ...
    for s = 1:exper.num.subNo % Subject
        ...
        condDat  = dlmread(strcat('dat_cond', num2str(c), '.csv'));
        subDat   = condDat(condDat(:, 1) == c & condDat(:, 2) == curr_sub, :);
        if isempty(subDat)
        ...
        [~, i] = unique(subDat(:, log.col.trialNo)); % Get trial #
        temp = subDat(i, :); % Matrix with only the first gaze shift per trial; used to extract fixed parameters from trial

        exper.trialNo(curr_sub, c)     = max(subDat(:, log.col.trialNo));    % # completed trials
        exper.excl_trials{curr_sub, c} = find(temp(:, log.col.fixErr) == 1); % Trials with fixation error
        perf.score.final(curr_sub, c)  = subDat(end, log.col.score);         % Score of subject at end of condition
        perf.hitMiss{curr_sub, c}      = temp(:, log.col.hitMiss);           % Hit/miss in trial
        stim.no_easyDis{curr_sub, c}   = temp(:, log.col.noDisEasy);         % # easy distractors in trial
        stim.no_hardDis{curr_sub, c}   = temp(:, log.col.noDisHard);         % # difficult distractors in trial
        clear i
        if c == 1
            test = temp(:, [log.col.gapPosEasy, log.col.gapPosHard]);
            if sum(temp(:, log.col.hitMiss)-(sum(test, 2, 'omitnan') == temp(:, log.col.gapPosReport))) ~= 0; keyboard; end
        elseif c == 2
            test = temp(:, [log.col.gapPosEasy, log.col.gapPosHard]);
            if sum(temp(:, log.col.hitMiss)-(any(test == temp(:, log.col.gapPosReport), 2))) ~= 0; keyboard; end
        end
        clear test
        ...
        for t = 1:no_trials_singleSub % Trial
            ...
            trial.events.stim_onOff(t, 1) = temp(t, 28);                       % Timestamp stimulus onset
            trial.events.stim_onOff(t, 2) = temp(t, 29);                       % Timestamp stimulus offset
            ...
            stim_locations = cat(3, temp(t, log.col.stimPosX), temp(t, log.col.stimPosY));

            % Get gaze shifts in trial
            % Gaze shifts are all saccades and blinks, detected in a trial
            li_trial = subDat(:, log.col.trialNo) == t;
            gazeShifts_singleTrial = subDat(li_trial, [3:4, 6:19]);
            clear li_trial
            ...
            if curr_cond == 3 % Double-target condition
                ...
                inp_gapLoc_easy = temp(t, log.col.gapPosEasy);
                inp_resp        = temp(t, log.col.gapPosReport);
            ...
            elseif curr_cond == 2 % Single-target condition
                ...
                choice_target(t) = temp(t, log.col.targetDiff);
                clear li_trial
            ...
            if choice_target(t) == stim.identifier(1, 1) % Easy chosen
                ...
                inspectedElements_no(t, 5) = temp(t, 21);

            elseif choice_target(t) == stim.identifier(1, 2) % Difficult chosen

                inspectedElements_no(t, 5) = temp(t, 22);
            ...
            gazeShifts_allTrials = [gazeShifts_allTrials; ...
                                    ...
                                    zeros(no_gs_ncs, 1)+temp(t, 21) ...                    Number easy distractors
                                    ...
                                    zeros(no_gs_ncs, 1)+temp(t, 22) ...                    Number difficult distractors
                                    zeros(no_gs_ncs, 1)+inspectedElements_no(t, 5)];     % Number distractors from chosen set
            clear no_gs_ncs gazeShifts_noConsec_singleTrial timelock li_noBg euc_dist stim_locations
        ...
        clear curr_sub trial gazeShifts_allTrials choice_target time_planning time_inspection time_decision subDat temp
        clear prop_gsClosest prop_gsFurther choice_congruence time_respBg inspectedElements_no time_trial li_atLeastOneGs

    end
    clear s curr_cond condDat
...
if plt.save == 1
    ...
    li_notExcld = ~cellfun(@isempty, sacc.gazeShifts(:, 1));
    inp_dat     = cellfun(@(x) x(:, [13, 15, 27]), ...
                          sacc.gazeShifts(li_notExcld, :), 'UniformOutput', false); % Get gaze shift endpoints and # difficult distractors in trials
    infSampling_plt_distanceDist(inp_dat, plt)
    clear li_notExcld inp_dat
...
%% Overall proportion gaze shifts to closest stimulus
% The main difference to the way we calculate proportion gaze shifts to the
% closest stimulus in the section above is that we are, here, first,
% calculate for each trial, what proportion of the gaze shifts in the trial
% targeted the closest stimulus. For each trial, this yields a vector with
% proportions, which is then averaged to obtain the overall proportion of a
% subject. In the section above, proportion gaze shifts to the closest
% stimulus is calculated, a), for each gaze shift in a trial and based on
% data from all trials, not individual trials
% sacc.propGs.closestFurther.overall = NaN(exper.num.subNo, exper.num.condNo, 2);
% for c = 1:exper.num.condNo % Condition
% 
%     curr_cond = exper.num.conds(c);
%     for s = 1:exper.num.subNo % Subject
...
%             dat_sub = dat_sub(~isnan(dat_sub(:, 23)), :);               % Excluded trials & trials without choice
% 
%             sacc.propGs.closestFurther.overall(curr_sub, c, 1) = mean(sacc.propGs.closest{curr_sub, c}, 'omitnan');
%             sacc.propGs.closestFurther.overall(curr_sub, c, 2) = mean(sacc.propGs.further{curr_sub, c}, 'omitnan');
% 
%         end
%         clear curr_sub dat_sub
% 
...
%     clear s
% 
%     % Plot overall proportion gaze shifts to closest stimulus vs proportion first gaze shifts to closest
%     if curr_cond == 3
% 
%         firstGs = cellfun(@(x) x(:, :, 3), vertcat(sacc.propGs.onCloser_trialBegin{:, 2}), 'UniformOutput', false);
%         firstGs = vertcat(firstGs{:});
%         firstGs = firstGs(firstGs(:, 1) == 1, 4:5);
% 
%         plt_dat    = {squeeze(sacc.propGs.closestFurther.overall(:, 2, :)); [NaN(2, 2); firstGs]}; % Add NaNs to make array size equal
...
% 
%         end
%         clear firstGs no_sp sp plt_dat plt_x_lab plt_y_lab plt_sp_tit
% 
...
% sacc.lat.firstGs_chosenSet   = NaN(exper.num.subNo, 2, exper.num.condNo);
% sacc.lat.firstGs_easySet     = NaN(exper.num.subNo, 2, exper.num.condNo);
% sacc.lat.firstGs_smallerSet  = NaN(exper.num.subNo, 2, exper.num.condNo);
% sacc.lat.firstGs_closestStim = NaN(exper.num.subNo, 2, exper.num.condNo);
% sacc.lat.firstGs_noDp        = NaN(exper.num.subNo, 2, 4);
% for c = 2:exper.num.condNo % Condition; only double-target
% 
%     for s = 1:exper.num.subNo % Subject
% 
...
%             dat_sub = dat_sub(~isnan(dat_sub(:, 23)), :);               % Excluded trials & trials without choice
% 
%             % Get stimulus category of interest, for which we want to analyse latencies
%             li_gsOnChosenSet   = any(dat_sub(:, 18) == stim.identifier(:, dat_sub(:, 23))', 2); % Set of chosen target
%             li_gsOnEasySet     = any(dat_sub(:, 18) == stim.identifier(:, 1)', 2);              % Easy set
%             li_gsOnClosestStim = dat_sub(:, 21) == 1;                                           % Closest stimulus
% 
...
%             inp_setSizes = dat_sub(:, 22);   % # easy distractors
% 
%             [~, sacc.lat.firstGs_chosenSet(curr_sub, 1:2, c), sacc.lat.firstGs_noDp(curr_sub, :, 1)] = ...
%                 infSampling_getLatFirstGs2(inp_latencis, inp_gsNos, inp_gsOnSOI, inp_trialNos, inp_noOI, inp_setSizes);
...
%                 infSampling_getLatFirstGs2(inp_latencis, inp_gsNos, inp_gsOnSOI, inp_trialNos, inp_noOI, inp_setSizes);
%             clear li_gsOnClosestStim 
% 
%             % Latencies of first gaze shift to smaller/larger set
%             dat_sub      = dat_sub(dat_sub(:, 22) ~= 4, :); % Excluded trials with equal # easy/difficult distractors
%             inp_latencis = dat_sub(:, 11);                  % Latencies
%             inp_gsNos    = dat_sub(:, 24);                  % Position of gaze shift in sequence
%             inp_trialNos = dat_sub(:, 26);                  % Trial numbers
%             inp_setSizes = dat_sub(:, 22);                  % # easy distractors
%             
%             inp_gsOnSOI = (any(dat_sub(:, 18) == stim.identifier(:, 1)', 2) & dat_sub(:, 22) >= 0 & dat_sub(:, 22) <= 3) | ...
%                           (any(dat_sub(:, 18) == stim.identifier(:, 2)', 2) & dat_sub(:, 22) >= 5 & dat_sub(:, 22) <= 8) ;
% 
%             [~, sacc.lat.firstGs_smallerSet(curr_sub, 1:2, c), ~] = ...
%                 infSampling_getLatFirstGs2(inp_latencis, inp_gsNos, inp_gsOnSOI, inp_trialNos, inp_noOI, inp_setSizes);
%             clear inp_latencis inp_gsNos inp_gsOnSOI inp_trialNos inp_noOI inp_setSizes
% 
%         end
%         clear curr_sub dat_sub
% 
...
cd('/Users/ilja/Dropbox/3_dissertation/2_informationSampling/1_hostedOnGit/3_analysis/_modelForZenodo');
% cd('/Users/i/Dropbox/3_dissertation/2_informationSampling/1_hostedOnGit/3_analysis/_modelForZenodo');
in_accuracy = [perf.hitrates(:,1,2), perf.hitrates(:,1,3)];
in_inspecTime = [sacc.time.mean.inspection(:,1,2)./1000, sacc.time.mean.inspection(:,1,3)./1000];
in_nonSearchTime = [sacc.time.mean.non_search(:,1,2)./1000, sacc.time.mean.non_search(:,1,3)./1000];

model_io = fit_idealObserverModel(in_accuracy, in_inspecTime, in_nonSearchTime);
clear in_accuracy in_inspecTime in_nonSearchTime
...
% end
all              = [];
all.containerDat = container_dat_mod;        % Get data from .xls files
all              = get_params(all);
all              = read_data(all);
% model_io              = fit_model(model_io);      % Fit model and plot results
all              = fit_regression(all); % Fit regression and plot results
% clear container_dat_mod


%% Fit probabilistic model
% Inputs
% inputPropChoicesEasyTarget: Proportion choices for easy target in the double target condition
% inputPropFixChosenSet:      Average proportion fixations per trial that landed on elements from the set of the eventually chosen target
% inputPropFixEasySet:        Average proportion fixations per trial that landed on elements from the easy set
% inputGainIdealObserver:     Ideal observer predictions for the gain of the easy and difficult target for different set size conditions
% inputProportionCorrect:     Empirical proportion correct for easy/difficult target (from single-target condition)
% inputProportionCorrect:     Empirical avearge inspection time for easy/difficult elements (from single-target condition)
% inputNonSearchTime:         Empirical avearge non-search time for easy/difficult elements (from single-target condition)
inputPropChoicesEasyTarget = stim.propChoice.easy(:, :, 2)'; 
inputPropFixChosenSet = sacc.propGs.onAOI_modelComparision_chosenNot_ss(:, :, 2);
inputPropFixEasySet = sacc.propGs.onAOI_modelComparision_easyDiff_ss(:, :, 2);
inputGainIdealObserver = model_io.gain.absolute;
inputProportionCorrect = [perf.hitrates(:, 1, 2), perf.hitrates(:, 1, 3)];
inputInspectionTime = [(sacc.time.mean.inspection(:, 1, 2)./1000), (sacc.time.mean.inspection(:, 1, 3)./1000)];
inputNonSearchTime = [(sacc.time.mean.non_search(:, 1, 2)./1000), (sacc.time.mean.non_search(:, 1, 3)./1000)];

model = probabilisticModel_main(inputPropChoicesEasyTarget, inputPropFixChosenSet, inputPropFixEasySet, inputGainIdealObserver, ...
                                inputProportionCorrect, inputInspectionTime, inputNonSearchTime);
clear inputPropChoicesEasyTarget inputPropFixChosenSet inputPropFixEasySet inputProportionCorrect inputInspectionTime inputNonSearchTime
...
% Figure 6
% Model results
clc; matlab_pairedTtest(all.data.double.perf, model_io.performance.perfect, 2)              % 6A: Empirical vs. maximum gain
[r, p, rl, ru] = corrcoef(all.data.double.perf, ...
                          model_io.performance.perfect, 'Rows', 'complete');
clc; disp(round([r(1, 2), rl(1, 2), ru(1, 2), p(1, 2)], 2));
clc; matlab_pairedTtest(model.freeParameter(:, 1), model.freeParameter(:, 2), 2)                % 6B: Free parameters distributions
clc; matlab_pairedTtest(all.data.double.perf, model.performance, 2)                        % 6C: Empirical vs. stochastic model gain
[r, p, rl, ru] = corrcoef(all.data.double.perf, model.performance, 'Rows', 'complete');
clc; disp(round([r(1, 2), rl(1, 2), ru(1, 2), p(1, 2)], 2));
...
emp_prop_choices_easy_fit = cat(3, all.reg.xn-1, all.reg.yn);
emp_slopesIntercepts      = all.reg.fit;
...
infSampling_plt_fig6_zenodo(inp_emp_propChoicesEasy, inp_emp_propGsChosen, inp_emp_perf, ...
                            model, model_io, plt)
...
prop_choices_easy_pred = model.propChoicesEasy';
...
propFix_pred = model.propFixChosen;
