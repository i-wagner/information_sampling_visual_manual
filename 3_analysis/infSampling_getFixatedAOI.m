function aoiVisit = infSampling_getFixatedAOI(endpoints_x, endpoints_y, stimLoc_x, stimLoc_y, aoi_radius, debug_plot)
    ...
    if debug_plot
        ...
        for s = 1:no_stim
            ...
            coord_aoi_x  = cos(unitCircle) .* aoi_radius + currStim_x;
            coord_aoi_y  = sin(unitCircle) .* aoi_radius + currStim_y;
            coord_stim_x = cos(unitCircle) .* 1.2512 + currStim_x;
            coord_stim_y = sin(unitCircle) .* 1.2512 + currStim_y;
        ...
        plot(cos(unitCircle).*2+0, sin(unitCircle).*2+0)                % Plot fixation tolerance area
