function events_gs = infSampling_getGazeShifts(gazeTrace, ts_stimOnset, minDur_gs, screen_x, screen_y)
    ...
    if ~isempty(blink_onsets)
        ...
        blink_onOff(:, end+1) = blink_onOff(:, 4) - ts_stimOnset;
        blink_onOff(:, end+1) = 2;
    ...
    % DEBUG
    % Check which gaze shifts are detected by one algorithm, but not the
...
%         % 
%         no_gs = size(events_gs2, 1);
%         if gs < no_gs & events_gs2(gs+1, 1) - events_gs2(gs, 2) <= 5 & ...
%            events_gs2(gs+1, 12) == events_gs2(gs, 12)
% 
%             keyboard;
%             temp = [events_gs2(gs, 1) events_gs2(gs+1, 2) ...
%                     events_gs2(gs, 3:6) events_gs2(gs+1, 7:9) ...
%                     events_gs2(gs+1, 2) - events_gs2(gs, 1) ...
%                     gazeTrace(events_gs2(gs, 1), 1) - ts_stimOnset ...
%                     events_gs2(gs, 12)];
% 
%             events_gs2(gs, :)  = temp;
%             events_gs2(gs+1, :) = [];
% 
%         elseif gs < no_gs & events_gs2(gs+1, 1) - events_gs2(gs, 2) <= 5 & ...
%                events_gs2(gs+1, 12) ~= events_gs2(gs, 12)
% 
%             keyboard
%             temp = [events_gs2(gs, 1) events_gs2(gs+1, 2) ...
...
%     clear gs no_gs no_ds
    ...
    % might have an extrem on-/offset, but a mean fixation location within
    % screen bounds: this could be indicate of head movements that moved
    % gaze around the screen. The latter case can be identified by checking
    % if the on-/offset is extreme, but mean fixation location is within
    % screen bounds
...
%                abs(events_gs(:, 15)) > screen_y & abs(events_gs(:, 15)) < 90;
    li_excld = events_gs(:, 10) < minDur_gs | ...     Gaze shift too short
               isnan(events_gs(:, 2)) | ...           Offset missing
               abs(events_gs(:, 5))  > screen_x | ... Onset/offset coordinate outside screen
               abs(events_gs(:, 8))  > screen_x | ...
               abs(events_gs(:, 13)) > screen_x | ...
               events_gs(:, 6)       > screen_y(1) | ...
               events_gs(:, 9)       > screen_y(1) | ...
               events_gs(:, 15)      > screen_y(1) | ...
               events_gs(:, 6)       < screen_y(2) | ...
               events_gs(:, 9)       < screen_y(2) | ...
               events_gs(:, 15)      < screen_y(2);
