function pos_all = infSampling_getStimLoc(locationsX, locationsY, noEasyDis, noHardDis, noOfTargets, inp_shownTarg)
    ...
    % In the single-target condition, # of distractors can be NaN: this is
    % because only one distractors type is shown each trial the # of
    % non-shown distractors is flagged with NaN (flagging them with 0, would
    % imply that zero distractors of the non-shown type were shown, which is
    % a seperate condition). To make life easier, we recode those cases to
    % 0 for purpose of extracting stimulus locations
    if isnan(noEasyDis)

        noEasyDis = 0;

    elseif isnan(noHardDis)
