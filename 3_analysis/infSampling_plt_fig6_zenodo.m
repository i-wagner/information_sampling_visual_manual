function [] = infSampling_plt_fig6(inp_emp_propChoicesEasy, inp_emp_propGsChosen, inp_emp_perf, ...
    ...
    gain_perfect = all.performance.perfect;
    gain_model   = model.performance;
    ...
    %% Model parameters
    x_jit = 0.075 .* randn(size(model.freeParameter, 1), 1);
    y_dat = fliplr(model.freeParameter);
    ...
    gain_perfect = all.performance.perfect;
    gain_model   = model.performance;         % Complex model
    dat_gain     = [gain_perfect gain_model]; 
    ...
    dat         = cat(3, inp_emp_propChoicesEasy, model.propChoicesEasy);
    ...
    propFix_pred_mean = mean(model.propFixChosen, 2);
