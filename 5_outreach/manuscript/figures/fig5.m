clear all; close all; clc

%% Get data
% Set folder
folder.root = '/Users/ilja/Dropbox/12_work/mr_informationSamplingVisualManual/';
folder.data = strcat(folder.root, '2_data/');
folder.fig = strcat(folder.root, "5_outreach/manuscript/figures/fig5");

data = load(strcat(folder.data, 'data_newPipeline.mat'));

%% Plot settings
% Assemble data for plot
plotDat = cat(3, ...
              [data.data.performance.gainPerTime(:,2), ...
               data.idealObserver.performance(:,2)], ...
              [data.data.performance.gainPerTime(:,4), ...
               data.idealObserver.performance(:,4)], ...
              [data.probabilisticModel.pred.visual.freeParameter(:,1), ...
               data.probabilisticModel.pred.manual.freeParameter(:,1)], ...
              [data.probabilisticModel.pred.visual.freeParameter(:,2), ...
               data.probabilisticModel.pred.manual.freeParameter(:,2)], ...
              [data.data.performance.gainPerTime(:,2), ...
               data.probabilisticModel.pred.visual.performance], ...
              [data.data.performance.gainPerTime(:,4), ...
               data.probabilisticModel.pred.manual.performance]);

% Define visuals
opt_visuals;
xLabels = ["Visual gain [cent/s]", "Manual gain [cent/s]", ...
           "Decision noise [visual]", "Fixation noise [visual]", ...
           "Visual gain [cent/s]", "Manual gain [cent/s]"];
yLabels = ["Ideal obs. gain [cent/s]", "Ideal obs. gain [cent/s]", ...
           "Decision noise [manual]", "Fixation noise [manual]", ...
           "Model gain [cent/s]", "Model gain [cent/s]"];

maxGain = plotDat(:,:,[1:2, 5:6]);
maxGain = ceil(max(maxGain(:)));
axLimits = [[0, maxGain]; [0, maxGain]; ...
            [0, 2]; [0, 2]; ...
            [0, maxGain]; [0, maxGain]];
axTicks = [{axLimits(1,1):0.50:axLimits(1,2)}, ...
           {axLimits(2,1):0.50:axLimits(2,2)}, ...
           {axLimits(3,1):0.50:axLimits(3,2)}, ...
           {axLimits(4,1):0.50:axLimits(4,2)}, ...
           {axLimits(5,1):0.50:axLimits(5,2)}, ...
           {axLimits(6,1):0.50:axLimits(6,2)}];

%% Plot
hFig = figure;
tiledlayout(3, 2);
for p = 1:size(plotDat, 3) % Panel
    if any(p == 1:4)
        plt.color.condition = plt.color.green;
    else
        plt.color.condition = plt.color.purple;
    end

    nexttile;
    line(axLimits(p,:), axLimits(p,:), ...
        'LineStyle', '-', ...
        'LineWidth', plt.line.widthThin, ...
        'Color', plt.color.black, ...
        'HandleVisibility', 'off');
    hold on
    plot(plotDat(:,1,p), plotDat(:,2,p), ...
        'o', ...
        'MarkerSize', plt.marker.sizeSmall, ...
        'MarkerFaceColor', plt.color.gray(2,:), ...
        'MarkerEdgeColor', plt.color.white, ...
        'LineWidth', plt.line.widthThin, ...
        'Color', plt.color.black)
    errorbar(mean(plotDat(:,1,p), 1, 'omitnan'), ...
             mean(plotDat(:,2,p), 1, 'omitnan'), ...
             ci_mean(plotDat(:,2,p)), ci_mean(plotDat(:,2,p)), ...
             ci_mean(plotDat(:,1,p)), ci_mean(plotDat(:,1,p)), ...
            'o', ...
            'MarkerSize', plt.marker.sizeLarge, ...
            'MarkerFaceColor', plt.color.black, ...
            'MarkerEdgeColor', 'none', ...
            'LineWidth', plt.line.widthThin, ...
            'CapSize', 0, ...
            'Color', plt.color.black, ...
            'HandleVisibility', 'off')
    [~, ~, h] = plotMean(plotDat(:,1,p), plotDat(:,2,p), plt.color.black);
    set(h(4), 'LineWidth', plt.line.widthThin);
    hold off
    axis([axLimits(p,:), axLimits(p,:)], 'square');
    xlabel(xLabels(p));
    ylabel(yLabels(p));
    xticks(axTicks{p});
    yticks(axTicks{p});
    box off
end
sublabel([], -10, -85);
opt.size = [25, 55];
opt.imgname = folder.fig;
opt.save = true;
prepareFigure(hFig, opt);
close;