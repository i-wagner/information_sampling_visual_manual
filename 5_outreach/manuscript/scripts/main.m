...
%% Time to complete conditions
% "conditionDurations" will be in minutes. To get a timestamp, add today's
% date (which has time component by default)
[mean(sum(time.conditionDurations , 2), 1, "omitnan") + datetime(datetime("today")), ...
 min(sum(time.conditionDurations , 2)) + datetime(datetime("today")), ...
 max(sum(time.conditionDurations , 2)) + datetime(datetime("today"))]
